...
class WAConnection extends events_1.EventEmitter {
    constructor() {
        ...
        this.connectOptions = {
            ...
            phoneResponseTime: 700,
            maxQueryResponseTime: 600,
        ...
        this.connectionDebounceTimeout = Utils.debouncedTimeout(800, () => this.state === 'connecting' && this.endConnection(Constants_1.DisconnectReason.timedOut));
        // tempo limite para saber quando terminamos de receber mensagens
        this.messagesDebounceTimeout = Utils.debouncedTimeout(1800);
        // ping chats para recebidos
        this.chatsDebounceTimeout = Utils.debouncedTimeout(9800);
        /**
    ...
    async unexpectedDisconnect(error) {
        if (this.state === 'open') {
            ...
            willReconnect && (this.connect()
                .catch(err => { }) // evitar exceção não tratada
            );
        }
        else {
    ...
    exitQueryIfResponseNotExpected(tag, cancel) {
        ...
        const listener = ({ connected }) => {
            if (connected) {
                ...
                this.off('conexão-telefone-mudança', listener);
            }
        };
        this.on('conexão-telefone-mudança', listener);
        return () => {
            this.off('conexão-telefone-mudança', listener);
    ...
    startPhoneCheckInterval() {
        ...
        if (!this.phoneCheckInterval) {
            // se já faz muito tempo e não temos notícias de WA, envie um ping
            this.phoneCheckInterval = setInterval(() => {
                ...
                if (this.phoneConnected !== false) {
                    this.phoneConnected = false;
                    this.emit('conexão-telefone-mudança', { connected: false });
                }
